...
var shouldPublish = 
        HasEnvironmentVariable("APPVEYOR_REPO_BRANCH")
            && HasEnvironmentVariable("NUGET_APIKEY")
...
Information("Will publish: {0}", shouldPublish);

// nuget parameters
var nugetApiKey = 
    HasEnvironmentVariable("NUGET_APIKEY") 
        ? EnvironmentVariable("NUGET_APIKEY") 
        : null;

if(!HasEnvironmentVariable("NUGET_APIKEY"))
{
...
Task("Publish")
    ...
    .Does(() =>
    {
        var settings = new DotNetCoreNuGetPushSettings
        {
            ...
            SymbolSource = "http://nuget.smbsrc.net"
        };
        var package = artifactsPath.Combine($"Mayhap.{nugetVersion}.nupkg");
